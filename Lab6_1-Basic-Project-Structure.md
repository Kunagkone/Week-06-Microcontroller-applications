...
ตอบ เข้าไปใน container ด้วยคำสั่ง docker run โดยใช้ image ที่มี ESP-IDF ติดตั้งไว้แล้ว จากนั้นใน container จะทำการตั้งค่าสภาพแวดล้อมโดยเรียก source export.sh เพื่อให้สามารถใช้คำสั่งของ ESP-IDF ได้ เช่น idf.py build

เมื่อรันคำสั่ง idf.py build ระบบจะเริ่มจากการตรวจสอบ dependencies และกำหนดค่าต่าง ๆ จาก sdkconfig จากนั้น CMake จะสร้าง build system และใช้ Ninja หรือ Make ทำการ compile source code ของโปรเจกต์รวมถึงไลบรารีของ ESP-IDF ให้กลายเป็น object files แล้วเชื่อมต่อ (link) ไฟล์ทั้งหมดเข้าเป็น binary
3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
ตอบ ทำหน้าที่ใน include($ENV{IDF_PATH}/tools/cmake/project.cmake)
4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
ตอบ อัปโหลดเข้าไปใน Git repository ซึ่งช่วยให้โปรเจกต์สะอาดและมีประสิทธิภาพมากขึ้น
5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
ตอบ ถ้า remove แล้ว run ใหม่ ข้อมูลภายใน container จะหาย ยกเว้นสิ่งที่ผูกกับ host
6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
ตอบ Docker เหมาะกับการพัฒนาแบบ พกพา, ควบคุมเวอร์ชัน, CI/CD, และ ทำงานเป็นทีม โดยไม่ต้องยุ่งกับระบบหลัก

Native system เหมาะกับ การเข้าถึง hardware โดยตรง, การทำงานร่วมกับ IDE หรือ GUI, และกรณีที่ไม่ต้องการเรียนรู้ Docker เพิ่มเติม
## 📋 ผลลัพธ์ที่คาดหวัง
